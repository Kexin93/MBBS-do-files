...
***DO FILE 12: TABLE M1-M2
***TREATMENT EFFECTS OF PARTNER INVITATION, BY WOMEN'S CON(DIS)CORDANCE BETWEEN METHOD USE AND TOP ATTRIBUTE AT BASELINE
...
* ============================== Concordant ================================
preserve
keep if method_attribute_concordance == 1 //Concordant
	eststo clear
* Column 1
...
esttab est1 est2 est3 est4 est5 using  "$output\allwomen_husband_ITT_ExAnteConcord.tex", replace fragment label nonumbers nolines cells(b(star fmt(%9.3f)) ///
...
prehead("\begin{table}\begin{center}\caption{Treatment Effect of the Partner Invitation Intervention, among Women who were Concordant between Method Use and Top Attribute at Baseline}\label{tab: allwomenhusbandITTConcordance}\tabcolsep=0.1cm\scalebox{0.72}{\begin{tabular}{lccccc}\toprule\multicolumn{6}{c}{\textbf{A. Stated Ideal Method and Method Use}}\\\midrule") ///
postfoot("\bottomrule") nogaps

esttab est6 est7 est8 est9 using  "$output\allwomen_husband_ITT_ExAnteConcord.tex", append fragment label nonumbers nolines cells(b(star fmt(%9.3f)) ///
se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N ymean, fmt(0 2) labels("N" "Control mean")) ///
...
* ============================== Discordant ================================
preserve
keep if method_attribute_concordance == 0
	eststo clear
* Column 1
eststo: reg diff_method_2 HUSB_T $covariates1 if COUN__FV_1 == 1, vce(robust) 
summarize diff_method_2 if HUSB_T == 0 & COUN__FV_1 == 1
...
esttab est1 est2 est3 est4 est5 using  "$output\allwomen_husband_ITT_ExAnteDiscordance.tex", replace fragment label nonumbers nolines cells(b(star fmt(%9.3f)) ///
se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N ymean, fmt(0 2) labels("N" "Control mean")) ///
mtitles("\makecell{Pre-Counseling and \\ Post-Counseling}" "\makecell{Counseling and \\ Follow-Up}" "\makecell{Counseling and \\ Follow-Up \\ (Adoption)}" "\makecell{Counseling and \\ Follow-Up \\ (Switching)}" "\makecell{Counseling and \\ Follow-Up \\ (Discontinuation)}") ///
mgroups("\makecell{Change to Stated Ideal Method \\ Between...}" "\makecell{Change in Method Use \\ Between...}", pattern(1 0 1 0 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///
prehead("\begin{table}\begin{center}\caption{Treatment Effect of the Partner Invitation Intervention, among Women who were Discordant between Method Use and Top Attribute at Baseline}\label{tab: allwomenhusbandITTDiscordance}\tabcolsep=0.1cm\scalebox{0.72}{\begin{tabular}{lccccc}\toprule\multicolumn{6}{c}{\textbf{A. Stated Ideal Method and Method Use}}\\\midrule") ///
postfoot("\bottomrule") nogaps

esttab est6 est7 est8 est9 using  "$output\allwomen_husband_ITT_ExAnteDiscordance.tex", append fragment label nonumbers nolines cells(b(star fmt(%9.3f)) ///
se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N ymean, fmt(0 2) labels("N" "Control mean")) ///
mtitles("\makecell{Stated Ideal Method \\after Counseling}" "\makecell{Stated Ideal Method \\ at FUP}" "\makecell{Stated Ideal Method \\ after Counseling}" "\makecell{Stated Ideal Method \\ at FUP}") ///
mgroups("\makecell{Whether Method Use at FUP \\ is Discordant with...}" "\makecell{Whether Method Use at Counseling \\ is Discordant with...}", pattern(1 0 1 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) ///

...
esttab est1 est2 est3 est4 using "$output\main_itt_results_surveymodes.rtf", replace label ///
starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(SHORT_T HUSB_T) ///
stats(N ymean, fmt(0 2) labels("N" "Control mean")) nomtitles cells("b(fmt(3) star) " se(par fmt(2)))  ///
title("Treatment Effect of Tailored Counseling and Partner Invitations") ///
refcat(SHORT_T "Panel A: Changes in Stated Preferred Method") collabels(none)
...
esttab est1 est2 est3 est4 using "$output\main_itt_results_surveymodes.rtf", append label nomtitles nonumbers starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(SHORT_T HUSB_T) cells("b(fmt(3) star) " se(par fmt(2))) ///
stats(N ymean, fmt(0 2) labels("N" "Control mean")) ///
refcat(SHORT_T "Panel B: Changes in Method Use") collabels(none)
...
esttab est1 est2 est3 est4 using "$output\main_itt_results_surveymodes.rtf", append label nomtitles nonumbers starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(SHORT_T HUSB_T) cells("b(fmt(3) star) " se(par fmt(2))) ///
stats(N ymean, fmt(0 2) labels("N" "Control mean"))  ///
refcat(SHORT_T "Panel C: Intertemporal Concordance") collabels(none)
...
// postfoot("\midrule Balancing controls &x & x & x & x \\ Survey mode FE & & x & & x \\\bottomrule \end{tabular}}\end{center}\footnotesize{Notes: Each panel presents regression results for the primary outcome that is specified. Columns (1) and (2) present results for the tailored counseling intervention, and columns (3) and (4) present results for the partner invitation intervention. Columns (1) and (3) control for baseline level balancing variables that include a woman's age, her contraceptive use, and whether her most valued attribute was contraceptive effectiveness. Columns (2) and (4) further control for the survey mode (clinic, home, or phone) at the follow-up. Variable definitions are described in Section 3 and are defined in more detail in Appendix A1. Heteroskedastic-robust standard errors are presented in brackets. ***1\%, ** 5\%, * 10\%.} \end{table}") nogaps

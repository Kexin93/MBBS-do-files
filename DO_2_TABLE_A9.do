...
eststo: reg diff_method_8 HUSB_T COUN_207 $covariates1 if COUN__FV_1 == 1 /*& !mi(FUP_curr_method)*/, vce(robust) 
summarize diff_method_8 if HUSB_T == 0 & COUN__FV_1 == 1 /*& !mi(FUP_curr_method)*/
estadd scalar ymean = r(mean)

* Panel A: pre-counseling to Follow-up ideal methods
esttab est1 est2 est3 est4 using "$output\allwomen_husband_to_ITT_mediationEffect207.tex", replace fragment label nolines ///
...
prehead("\begin{table}\begin{center}\caption{Treatment Effect of the Partner Invitation Intervention with Partner Participation being the Mediator}\label{tab: allwomenhusbandITTmediation}\tabcolsep=0.1cm\scalebox{0.8}{\begin{tabular}{lcccc}\toprule") ///
...
eststo: reg diff_method_3 HUSB_T COUN_207 $covariates1, vce(robust) 
summarize diff_method_3 if HUSB_T == 0
estadd scalar ymean = r(mean)

*Panel B: Counseling current method and Follow-up current method
esttab est1 est2 est3 est4 using "$output\allwomen_husband_to_ITT_mediationEffect207.tex", append fragment label nomtitles nonumbers nolines cells(b(star fmt(%9.3f)) ///
...
eststo: reg diff_method_9 HUSB_T COUN_207 $covariates1, vce(robust) 
summarize diff_method_9 if HUSB_T == 0
estadd scalar ymean = r(mean)
...
eststo: reg diff_method_5 HUSB_T COUN_207 $covariates1 if COUN__FV_1 == 1, vce(robust) 
summarize diff_method_5 if HUSB_T == 0 & COUN__FV_1 == 1
estadd scalar ymean = r(mean)

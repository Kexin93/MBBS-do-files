...
******************* 1. PANEL A: Husband Invitation on End-of-Counseling Ideal Method **********************************************
	eststo clear
	eststo: reg postIdeal_implants HUSB_T $covariates if /*!mi(FUP_curr_method) &*/ !mi(COUN_3081), vce(robust) 
	summarize postIdeal_implants if HUSB_T == 0 & /*!mi(FUP_curr_method) &*/ !mi(COUN_3081)
	estadd scalar ymean = r(mean)

	eststo: reg postIdeal_injectables HUSB_T $covariates if /*!mi(FUP_curr_method) &*/ !mi(COUN_3081), vce(robust) 
	summarize postIdeal_injectables if HUSB_T == 0 & /*!mi(FUP_curr_method) &*/ !mi(COUN_3081)
	estadd scalar ymean = r(mean)

	eststo: reg postIdeal_pills HUSB_T $covariates if /*!mi(FUP_curr_method) &*/ !mi(COUN_3081), vce(robust) 
	summarize postIdeal_pills if HUSB_T == 0 & /*!mi(FUP_curr_method) &*/ !mi(COUN_3081)
	estadd scalar ymean = r(mean)

	eststo: reg postIdeal_Others HUSB_T $covariates if /*!mi(FUP_curr_method) &*/ !mi(COUN_3081), vce(robust) 
...
prehead("\begin{table}[h!]\begin{center}\caption{Treatment Effect of the Partner Invitation by Method Type}\label{tab: mechanisms4methodsITT}\tabcolsep=0.3cm\scalebox{0.68}{\begin{tabular}{lcccc}\toprule") ///
...
* ============================ 4. FUP ideal Method = injectables ========================================
	eststo clear
eststo: reg FUPIdeal_implants HUSB_T $covariates if /*!mi(FUP_curr_method) &*/ !mi(COUN_3081), vce(robust) 
summarize FUPIdeal_implants if HUSB_T == 0 & /*!mi(FUP_curr_method) &*/ !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPIdeal_injectables HUSB_T $covariates if /*!mi(FUP_curr_method) &*/ !mi(COUN_3081), vce(robust) 
summarize FUPIdeal_injectables if HUSB_T == 0 & /*!mi(FUP_curr_method) &*/ !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPIdeal_pills HUSB_T $covariates if /*!mi(FUP_curr_method) &*/ !mi(COUN_3081), vce(robust) 
summarize FUPIdeal_pills if HUSB_T == 0 & /*!mi(FUP_curr_method) &*/ !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPIdeal_Others HUSB_T $covariates if /*!mi(FUP_curr_method) &*/ !mi(COUN_3081), vce(robust) 

...
esttab discuss_kids_husb_Y1 discuss_kids_husb_Y2 discuss_kids_husb_Y3 discuss_kids_husb_Y4 using "$output\partner_subgroups4GP.tex", replace fragment label nolines ///
...
esttab discuss_kids_husb_N1 discuss_kids_husb_N2 discuss_kids_husb_N3 discuss_kids_husb_N4 using "$output\partner_subgroups4GP.tex", append fragment label nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N, fmt(0) labels("N"))  ///
coeflabel(HUSB_T "\makecell[l]{Did not discuss \\ children with husband}") nomtitles collabels(none) nonumbers postfoot("\midrule")

esttab cont_wom_decide_Y1 cont_wom_decide_Y2 cont_wom_decide_Y3 cont_wom_decide_Y4 using "$output\partner_subgroups4GP.tex", append fragment label nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N, fmt(0) labels("N"))  ///
coeflabel(HUSB_T "\makecell[l]{Woman decides \\ FP use/non-use}") nomtitles collabels(none) nonumbers postfoot("\cdashline{1-5}")

esttab cont_wom_decide_N1 cont_wom_decide_N2 cont_wom_decide_N3 cont_wom_decide_N4 using "$output\partner_subgroups4GP.tex", append fragment label nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N, fmt(0) labels("N"))  ///
coeflabel(HUSB_T "\makecell[l]{Woman does not decide \\ FP use/non-use}") nomtitles collabels(none) nonumbers postfoot("\midrule")

esttab husb_supports_fp_Y1 husb_supports_fp_Y2 husb_supports_fp_Y3 husb_supports_fp_Y4 using "$output\partner_subgroups4GP.tex", append fragment label nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N, fmt(0) labels("N"))  ///
coeflabel(HUSB_T "Husband supports FP") nomtitles collabels(none) nonumbers postfoot("\cdashline{1-5}")

esttab husb_supports_fp_N1 husb_supports_fp_N2 husb_supports_fp_N3 husb_supports_fp_N4 using "$output\partner_subgroups4GP.tex", append fragment label nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N, fmt(0) labels("N"))  ///
coeflabel(HUSB_T "Husband does not support FP") nomtitles collabels(none) nonumbers postfoot("\midrule")

esttab husb_sat_Y1 husb_sat_Y2 husb_sat_Y3 husb_sat_Y4 using "$output\partner_subgroups4GP.tex", append fragment label nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N, fmt(0) labels("N"))  ///
...
esttab husb_sat_N1 husb_sat_N2 husb_sat_N3 husb_sat_N4 using "$output\partner_subgroups4GP.tex", append fragment label nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N, fmt(0) labels("N"))  ///

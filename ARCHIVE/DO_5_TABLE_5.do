...
***DO FILE 5: TABLE 5 (change within LARCs)
...
eststo: reg diff_method_ideal_LARC SHORT_T, vce(robust) 
summarize diff_method_ideal_LARC if SHORT_T == 0  
estadd scalar ymean = r(mean)
	
eststo: reg diff_method_ideal_LARC SHORT_T $balance_covariates, vce(robust) 
summarize diff_method_ideal_LARC if SHORT_T == 0  
estadd scalar ymean = r(mean)
	
eststo: reg diff_method_ideal_LARC HUSB_T, vce(robust) 
summarize diff_method_ideal_LARC if HUSB_T == 0  
estadd scalar ymean = r(mean)

eststo: reg diff_method_ideal_LARC HUSB_T $balance_covariates, vce(robust) 
summarize diff_method_ideal_LARC if HUSB_T == 0  
estadd scalar ymean = r(mean)
	
* Panel A: pre-counseling to Follow-up ideal methods
esttab est1 est2 est3 est4 using "$output\LARC_results.tex", replace fragment label nolines ///
...
prehead("\begin{table}\begin{center}\caption{Treatment Effect of Tailored Counseling and Partner invitations (for LARCs only)}\label{tab: allwomenLARCs}\tabcolsep=0.3cm\scalebox{0.85}{\begin{tabular}{lcccc}\toprule") ///
...
******************* PANEL B. Counseling current method and FUP current method**********************************************	
	eststo clear
eststo: reg diff_method_LARC_use SHORT_T, vce(robust) 
summarize diff_method_LARC_use if SHORT_T == 0 
estadd scalar ymean = r(mean)
	
eststo: reg diff_method_LARC_use SHORT_T $balance_covariates, vce(robust) 
summarize diff_method_LARC_use if SHORT_T == 0 
estadd scalar ymean = r(mean)

eststo: reg diff_method_LARC_use HUSB_T, vce(robust) 
summarize diff_method_LARC_use if HUSB_T == 0
estadd scalar ymean = r(mean)

eststo: reg diff_method_LARC_use HUSB_T $balance_covariates, vce(robust) 
summarize diff_method_LARC_use if HUSB_T == 0
estadd scalar ymean = r(mean)

*Panel B: Counseling current method and Follow-up current method
esttab est1 est2 est3 est4 using "$output\LARC_results.tex", append fragment label nomtitles nonumbers nolines cells(b(star fmt(%9.3f)) ///
...
******************* Panel C: post-COUN ideal method and FUP current method**********************************************
	eststo clear
eststo: reg diff_LARC_ID SHORT_T, vce(robust) 
summarize diff_LARC_ID if SHORT_T == 0
estadd scalar ymean = r(mean)

eststo: reg diff_LARC_ID SHORT_T $balance_covariates, vce(robust) 
summarize diff_LARC_ID if SHORT_T == 0
estadd scalar ymean = r(mean)

eststo: reg diff_LARC_ID HUSB_T, vce(robust) 
summarize diff_LARC_ID if HUSB_T == 0
estadd scalar ymean = r(mean)

eststo: reg diff_LARC_ID HUSB_T $balance_covariates, vce(robust) 
summarize diff_LARC_ID if HUSB_T == 0
estadd scalar ymean = r(mean)
...
******************* Panel D: FUP ideal method and FUP current method**********************************************
	eststo clear
eststo: reg diff_LARC_CD SHORT_T , vce(robust) 
summarize diff_LARC_CD if SHORT_T == 0 
estadd scalar ymean = r(mean)

eststo: reg diff_LARC_CD SHORT_T $balance_covariates , vce(robust) 
summarize diff_LARC_CD if SHORT_T == 0 
estadd scalar ymean = r(mean)

eststo: reg diff_LARC_CD HUSB_T , vce(robust) 
summarize diff_LARC_CD if HUSB_T == 0 
estadd scalar ymean = r(mean)
	
eststo: reg diff_LARC_CD HUSB_T $balance_covariates , vce(robust) 
summarize diff_LARC_CD if HUSB_T == 0 
estadd scalar ymean = r(mean)

...
***DO FILE 11: TABLE L2
***TREATMENT EFFECT OF THE PARTNER INVITATION INTERVENTION BY METHOD TYPE, AMONG WOMEN WHO WERE ASSIGNED TO THE SHORT, TAILORED COUNSELING INTERVENTION
...
******************* 1. PANEL A: Husband Invitation on End-of-Counseling Ideal Method **********************************************
	eststo clear														
	eststo: reg postIdeal_implants HUSB_T $covariates if !mi(COUN_3081), vce(robust) 
	summarize postIdeal_implants if HUSB_T == 0 & !mi(COUN_3081)
	estadd scalar ymean = r(mean)

	eststo: reg postIdeal_injectables HUSB_T $covariates if !mi(COUN_3081), vce(robust) 
	summarize postIdeal_injectables if HUSB_T == 0 & !mi(COUN_3081)
	estadd scalar ymean = r(mean)

	eststo: reg postIdeal_pills HUSB_T $covariates if !mi(COUN_3081), vce(robust) 
	summarize postIdeal_pills if HUSB_T == 0 & !mi(COUN_3081)
	estadd scalar ymean = r(mean)

	eststo: reg postIdeal_Others HUSB_T $covariates if !mi(COUN_3081), vce(robust) 
	summarize postIdeal_Others if HUSB_T == 0 & !mi(COUN_3081)
	estadd scalar ymean = r(mean)

esttab est1 est2 est3 est4 using "$output\allwomen_husband_bymethod_SHORT.tex", replace fragment label nolines ///
...
prehead("\begin{table}[h!]\begin{center}\caption{Treatment Effect of the Partner Invitation Intervention by Method Type, among Women who were Assigned to the Short, Tailored Counseling Intervention}\label{tab: mechanisms4methodsITTinShort}\tabcolsep=0.3cm\scalebox{0.68}{\begin{tabular}{lcccc}\toprule") ///
...
*========================= 2. Follow-up Method Use =======================================
	eststo clear
eststo: reg FUPmethod_implants HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize FUPmethod_implants if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPmethod_injectables HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize FUPmethod_injectables if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPmethod_pills HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize FUPmethod_pills if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPmethod_Others HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize FUPmethod_Others if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

esttab est1 est2 est3 est4 using "$output\allwomen_husband_bymethod_SHORT.tex", append nomtitles nonumbers fragment label nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N ymean, fmt(0 2) labels("N" "Control mean"))  ///
...
* ============================== 3. End-of-Counseling and FUP Method Use both being Implants =====================================
	eststo clear
eststo: reg uptake_implants HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize uptake_implants if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg uptake_injectables HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize uptake_injectables if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg uptake_pills HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize uptake_pills if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg uptake_Others HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
...
esttab est1 est2 est3 est4 using "$output\allwomen_husband_bymethod_SHORT.tex", append fragment nomtitles label nonumbers nolines cells(b(star fmt(%9.3f)) ///
...
* ============================ 4. FUP ideal Method = injectables ========================================
	eststo clear
eststo: reg FUPIdeal_implants HUSB_T $covariates if !mi(COUN_3081), vce(robust) 
summarize FUPIdeal_implants if HUSB_T == 0 & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPIdeal_injectables HUSB_T $covariates if !mi(COUN_3081), vce(robust) 
summarize FUPIdeal_injectables if HUSB_T == 0 & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPIdeal_pills HUSB_T $covariates if !mi(COUN_3081), vce(robust) 
summarize FUPIdeal_pills if HUSB_T == 0 & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg FUPIdeal_Others HUSB_T $covariates if !mi(COUN_3081), vce(robust) 
summarize FUPIdeal_Others if HUSB_T == 0 & !mi(COUN_3081)
estadd scalar ymean = r(mean)
...
* ====================== 5. Concordance at the FUP between ideal method and method use =========================================
	eststo clear
eststo: reg concordance_implants HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize concordance_implants if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg concordance_injectables HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize concordance_injectables if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg concordance_pills HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
summarize concordance_pills if HUSB_T == 0 & !mi(FUP_curr_method) & !mi(COUN_3081)
estadd scalar ymean = r(mean)

eststo: reg concordance_Others HUSB_T $covariates if !mi(FUP_curr_method) & !mi(COUN_3081), vce(robust) 
...
esttab est1 est2 est3 est4 using "$output\allwomen_husband_bymethod_SHORT.tex", append fragment label nomtitles nonumbers nolines ///
cells(b(star fmt(%9.3f)) se(par( [ ] ) fmt(%9.3f))) starlevels(* 0.2 ** 0.1 *** 0.02) compress style(tab) keep(HUSB_T) ///
stats(N ymean, fmt(0 2) labels("N" "Control mean"))  ///

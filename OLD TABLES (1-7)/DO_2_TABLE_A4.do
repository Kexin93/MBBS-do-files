...
eststo: ivregress 2sls diff_method_8 (COUN_207 = HUSB_T) $covariates1 if COUN__FV_1 == 1 /*& !mi(FUP_curr_method)*/, vce(robust)
summarize diff_method_8 if HUSB_T == 0 & COUN__FV_1 == 1 /*& !mi(FUP_curr_method)*/
estadd scalar ymean = r(mean)
	estat firststage
	matrix FS = r(singleresults)
...
esttab est1 est2 est3 est4 using "$output\allwomen_husband_toIV_IV.tex", replace fragment label nolines ///
...
prehead("\begin{table}\begin{center}\caption{Instrumental Variable (IV-2SLS) Regression Results of Partner Invitation Intervention}\label{tab: allwomenhusbandIV}\tabcolsep=0.1cm\scalebox{0.88}{\begin{tabular}{lcccc}\toprule") ///
...
eststo: ivregress 2sls diff_method_3 (COUN_207 = HUSB_T) $covariates1, vce(robust)
summarize diff_method_3 if HUSB_T == 0
estadd scalar ymean = r(mean)
	estat firststage
	matrix FS = r(singleresults)
...
esttab est1 est2 est3 est4 using "$output\allwomen_husband_toIV_IV.tex", append fragment label nomtitles nonumbers nolines cells(b(star fmt(%9.3f)) ///
...
eststo: ivregress 2sls diff_method_9 (COUN_207 = HUSB_T) $covariates1, vce(robust)
summarize diff_method_9 if HUSB_T == 0
estadd scalar ymean = r(mean)
	estat firststage
	matrix FS = r(singleresults)

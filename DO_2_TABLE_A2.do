...
******************************************************************************
***************** TABLE A3, COLUMN 3: Husband Ideal Method *****************************************
************************************************************************	
...
esttab baseline_curr_method baseline_ideal_method husb_ideal_method using "$output\baseline_curr_method.tex", booktabs fragment ///
label cells("mean(pattern(1) fmt(2))") ///
replace collabels(none) compress style(tab) ///
mtitles("Current Method" "Woman Ideal" "Husband Ideal") ///
prehead("\begin{table}\begin{center} \caption{Distribution of Method Use and Stated Ideal Contraceptive Method at Baseline}\label{tab: baselineCurrIdealhusbIdeal}\tabcolsep=0.1cm\begin{tabular}{lccc}\toprule") ///
postfoot("\bottomrule\end{tabular}\end{center}\footnotesize{Note: Column (1) shows the current method use at baseline among 777 women (679 current users + 98 non-users). Column (2) displays women's ideal method at baseline among 773 women (679 current users + 88 non-users who will pick up a method in the future + 6 non-users who will not pick up any method in the future). Column (3) presents the distribution of male partner's reported ideal contraceptive method among the 112 husbands who were present at the counseling session.}\end{table}")
